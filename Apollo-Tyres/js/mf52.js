...
document.getElementById('submitBtn').addEventListener('click', async function() {
    ...
    // ⛔ DO NOT call saveDraft with an undefined matrix here.
    // Draft with matrix will be saved after we compute extractedData in processMF52Excel().
    ...
    .then(data => {
        ...
        return fetch('/api/read-output-excel')
            ...
            .then(data => {
                ...
                // ✅ Save a draft with the computed matrix BEFORE storing to DB / redirect
                const matrixJson = {
                  protocol: 'MF52',
                  generatedAt: new Date().toISOString(),
                  rows: extractedData
                };
                // fire and forget; if it fails, it shouldn't block the user
                saveDraft('MF52', matrixJson).catch(() => {});
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (pid) {
    try {
      ...
      if (proj && proj.project && proj.project.inputs) prefill(proj.project.inputs);
    } catch (e) { /* ignore */ }
    if (pid) {

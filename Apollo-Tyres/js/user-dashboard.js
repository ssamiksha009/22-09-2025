...
async function loadProjects() {
  ...
  // Show 7 most recent by "last activity"; pinned rows float to top of those
  ...
  for (const p of top7) {
    ...
    const statusText  = String(p.status || 'not started');
    const statusLower = statusText.toLowerCase();
    const isInProgress = (statusLower === 'in progress');

    const actionsHtml = isInProgress
      ? `<button class="btn btn-primary" data-continue>Continue</button>`
    ...
    // Actions
    if (isInProgress) {
      const btn = tr.querySelector('[data-continue]');
      btn.addEventListener('click', () => {
        // mark "touched" so it floats in recents
        try { touchProject(p.id); } catch {}
        const pid = encodeURIComponent(p.id ?? p.project_name ?? '');
        const protocol = encodeURIComponent(p.protocol || '');
        // Route to select.html with resume=1 so it loads prior matrix + right-side Tydex list
        window.location.href = `/select.html?projectId=${pid}&protocol=${protocol}&resume=1`;
      });
    } else {
...
// Map DB protocol → page (kept for non-In-Progress flows)
...
// Decide where to go when “Open Project” is clicked (non-In-Progress)
function openProject(p) {
  ...
  if (status === 'in progress' && pid) {
    // If someone clicks "Open Project" on an in-progress row (shouldn't happen now),
    // keep the behavior consistent with Continue and go to select.html as well.
    const protocol = encodeURIComponent(p.protocol || '');
    window.location.href = `/select.html?projectId=${pid}&protocol=${protocol}&resume=1`;
  } else if (pid) {
    // For completed or others, you may keep going to protocol pages or a viewer
    window.location.href = `/${page}?projectId=${pid}`;
  } else {
  ...
  <div class="recent-item-card">
    <div class="recent-left">
      <div class="recent-icon">
        <svg width="16" height="16" viewBox="0 0 24 24"><path fill="#4CAF50" d="M12 0L15.09 7.36L23 8.63L17 14.14L18.18 22L12 18.77L5.82 22L7 14.14L1 8.63L8.91 7.36L12 0Z"/></svg>
      </div>
      <div class="recent-info">
        <div class="recent-title">${escapeHtml(p.project_name || p.id)}</div>
        <div class="recent-meta">
          <span class="project-badge">${escapeHtml(p.protocol || '')}</span> · ${fmtDateTime(p.created_at)} · <span class="relative-time">${timeAgo(p.created_at)}</span>
        </div>
      </div>
    </div>

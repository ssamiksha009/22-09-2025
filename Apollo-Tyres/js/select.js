...
// Map protocol name (URL param) -> scratch endpoint + UI title + table toggle
function mapProtocolToFetchEndpoint(protoName) {
  const key = String(protoName || '').trim().toUpperCase();
  switch (key) {
    case 'MF62':   return { url: '/api/get-mf-data',    title: 'MF 6.2 Protocol', tableId: 'mf62Table' };
    case 'MF52':   return { url: '/api/get-mf52-data',  title: 'MF 5.2 Protocol', tableId: 'mf52Table' };
    case 'FTIRE':  return { url: '/api/get-ftire-data', title: 'FTire Protocol',  tableId: 'ftireTable' };
    case 'CDTIRE': return { url: '/api/get-cdtire-data',title: 'CDTire Protocol', tableId: 'cdtireTable' };
    case 'CUSTOM': return { url: '/api/get-custom-data',title: 'Custom Protocol',  tableId: 'customTable' };
    default:       return null;
  }
}

function showOnlyTable(tableId, titleText) {
  const ids = ['mf62Table','mf52Table','ftireTable','cdtireTable','customTable'];
  ids.forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.display = (id === tableId) ? 'table' : 'none';
  });
  const titleEl = document.getElementById('protocol-title');
  if (titleEl) titleEl.textContent = titleText || '';
}

async function loadPreviousTydex(projectId, protocol) {
  try {
    const res = await fetchJSON(`/api/projects/${encodeURIComponent(projectId)}/protocol/${encodeURIComponent(protocol)}/tydex`);
    const list = document.getElementById('previous-tydex-list');
    if (!list) return;
    list.innerHTML = '';

    const files = res.files || [];
    if (!files.length) {
      list.innerHTML = '<li class="muted">No Tydex files yet.</li>';
      return;
    }
    ...
    files.forEach(f => {
      ...
      left.textContent = f.file_name;

      const btn = document.createElement('button');
      btn.className = 'btn btn-sm';
      btn.textContent = 'View / Download';
      btn.addEventListener('click', () => {
        window.open(`/api/projects/${encodeURIComponent(projectId)}/protocol/${encodeURIComponent(protocol)}/tydex/${encodeURIComponent(f.id)}`, '_blank');
      });

      const ts = document.createElement('span');
      ts.className = 'muted';
      ts.style.marginLeft = '8px';
      ts.textContent = new Date(f.created_at).toLocaleString();
    ...
    console.warn('Previous Tydex load failed:', e.message || e);
  }
}
...
// protocol key for server scratch-table helpers: 'mf62' | 'mf52' | 'ftire' | 'cdtire' | 'custom'
function getVisibleProtocolKey() {
  const t = visibleDataTable();
  if (!t) return null;
  return t.id.replace('Table', ''); // e.g. mf62Table -> 'mf62'
}

// protocol name for TYDEX generator: 'MF6pt2' | 'MF5pt2' | 'FTire' | 'CDTire' | 'Custom'
...
// Resolve project id from URL, session, or by asking server (fallback)
async function findProjectId() {
  ...
  // c) server lookup by project name (last resort)
...
async function updateStatusIndicators() {
  ...
  const protocolKey = getVisibleProtocolKey(); // 'mf62' | 'mf52' | ...
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  const urlProjId  = qs.get('projectId');
  const urlProto   = qs.get('protocol'); // 'MF62' | 'MF52' | 'FTire' | 'CDTire' | 'Custom'
  ...
  [mf62Table, mf52Table, ftireTable, cdtireTable, customTable].forEach(t => { if (t) t.style.display = 'none'; });

  // ======== RESUME MODE (from Dashboard "Continue") ========
  if (resume === '1' && urlProjId && urlProto) {
    try {
      // 1) Load project meta to set name/id for status + tydex helpers
      const meta = await fetchJSON(`/api/projects/${encodeURIComponent(urlProjId)}`);
      if (!meta || !meta.project) throw new Error('Project not found');
      const proj = meta.project;
      sessionStorage.setItem('currentProject', proj.project_name || '');
      sessionStorage.setItem('currentProjectId', String(proj.id));

      // 2) Show correct protocol table and set title
      const map = mapProtocolToFetchEndpoint(urlProto);
      if (!map) throw new Error(`Unknown protocol: ${urlProto}`);

      showOnlyTable(map.tableId, map.title);

      // 3) Fetch scratch rows from temp table (already computed previously)
      const data = await fetchJSON(map.url);

      if (map.tableId === 'mf62Table')      displayMF62Data(data);
      else if (map.tableId === 'mf52Table') displayMF52Data(data);
      else if (map.tableId === 'ftireTable')displayFTireData(data);
      else if (map.tableId === 'cdtireTable')displayCDTireData(data);
      else                                  displayCustomData(data);

      // 4) Update per-row status and buttons
      await updateStatusIndicators();

      // 5) Populate the right-side Tydex sidebar from DB
      await loadPreviousTydex(urlProjId, urlProto);
    } catch (e) {
      console.error('Resume flow failed:', e);
      document.getElementById('data-container').innerHTML =
        `<p class="error-message">Could not resume this project: ${escapeHtml(e.message || e)}</p>`;
    }
    // Important: stop here, don't run the old referer-based loader
    return;
  }

  // ======== ARCHIVED MODE: open a completed project by id (your existing block) ========
  const archivedProjectId = qs.get('projectId');
  if (archivedProjectId) {
    (async () => {
      try {
        // 1) Load project meta and stash name/id for status/TYDEX helpers
        const metaRes = await fetch(`/api/projects/${encodeURIComponent(archivedProjectId)}`);
        const meta = await metaRes.json();
        const ARCHIVED_PROJECT = meta.project; // keep for later (inputs, protocol, etc.)

        if (!meta.success || !meta.project) throw new Error('Project not found');

        const { id, project_name } = meta.project;
        sessionStorage.setItem('currentProject', project_name);
        sessionStorage.setItem('currentProjectId', String(id));

        // 2) Load archived rows
        const dataRes = await fetch(`/api/projects/${encodeURIComponent(archivedProjectId)}/data`);
        const dataJson = await dataRes.json();
        if (!dataJson.success) throw new Error('Failed to load archived data');

        const archivedProtocol = dataJson.protocol;  // 'MF62' | 'MF52' | 'FTire' | 'CDTire' | 'Custom'
        const rows = dataJson.rows || [];

        // 3) Hide all tables first
        ...
        switch (archivedProtocol) {
          case 'MF62': show(mf62Table, 'MF 6.2 Protocol', displayMF62Data); break;
          case 'MF52': show(mf52Table, 'MF 5.2 Protocol', displayMF52Data); break;
          ...
          default: throw new Error(`Unknown protocol: ${archivedProtocol}`);
        }

        // 4) Read-only UI for archived view: hide run controls and mark-in-progress
        document.querySelectorAll('.run-button-cell, .row-run-btn').forEach(n => { n.style.display = 'none'; });
        ['completeProjectBtn', 'runAllBtn', 'markInProgressBtn'].forEach(id => {
          const el = document.getElementById(id);
          if (el) el.style.display = 'none';
        });

        // 5) Insert archived-only toolbar (Back / Export CSV / View Inputs)
        initArchivedToolbar({
          projectId: id,
...
function setProtocolTitle(text) {
  const el = document.getElementById('protocol-title');
...
async function generateTydex(runNumber) {
  ...
  try {
    ...
    if (result.success) {
      ...
      // If we have projectId + protocol in URL, refresh sidebar list (in case backend also saved it)
      const qs = new URLSearchParams(location.search);
      const pid = qs.get('projectId');
      const proto = qs.get('protocol');
      if (pid && proto) {
        try { await loadPreviousTydex(pid, proto); } catch {}
      }
    } else {
...
/* Optional global generator (kept from your code) */
document.addEventListener('click', function (e) {
  if (e.target && e.target.classList.contains('tydex-btn')) {
    const button = e.target;
    if (button.classList.contains('open-file')) return; // already wired to open
...
// Complete: archive + mark completed
document.getElementById('completeProjectBtn').addEventListener('click', async function () {
  ...
  try {
    ...
    alert('Archived and marked Completed!');
...
/* ---------------- archived loader (utility if needed) ---------------- */

async function loadCompletedProject(projectId) {
  try {
    const res = await fetch(`/api/projects/${encodeURIComponent(projectId)}/data`);
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      alert('Failed to load archived data: ' + (err.message || res.statusText));
      return;
    }
    const j = await res.json();
    if (!j.success) { alert('Failed to load archived data'); return; }
    const { protocol, rows } = j;

    if (protocol === 'MF62') displayMF62Data(rows);
    else if (protocol === 'MF52') displayMF52Data(rows);
    else if (protocol === 'FTire') displayFTireData(rows);
    else if (protocol === 'CDTire') displayCDTireData(rows);
    else displayCustomData(rows);

    updateStatusIndicators();
  } catch (error) {
    console.error('loadCompletedProject error', error);
    alert('Error loading archived project: ' + (error.message || error));
  }
}
...
function initArchivedToolbar({ projectId, projectName, protocol, rows }) {
  ...
  if (!bar) {
    ...
    // Place right under the main header if it exists, else at top of body
...
/*
  Enhanced "View Inputs" modal + helpers.
  - showInputsModal(arg) accepts either a projectId (string/number) or an inputs object (or a project object).
  - INPUT_LABELS / INPUT_ORDER control label mapping and display order.
*/
const INPUT_LABELS = {
...
/*
  Show inputs helper:
  - If arg is projectId (string/number) -> fetch project and display inputs.
  - If arg is an object (project or inputs) -> render directly.
*/
async function showInputsModal(arg) {
  // If arg is falsy, do nothing
  if (!arg) return;

  // If arg looks like an id (string/number), fetch project
  if (typeof arg === 'string' || typeof arg === 'number') {
    try {
      ...
      // inputs might be JSON string
      if (typeof inputs === 'string') {
        try { inputs = JSON.parse(inputs); } catch (_) { /* keep string */ }
      }
  ...
  // If arg is an object — could be { project: {...} } or inputs directly
  let inputs = arg.inputs || arg.project?.inputs || arg;
  let protocol = arg.protocol || arg.project?.protocol || '';
  if (typeof inputs === 'string') {
    try { inputs = JSON.parse(inputs); } catch (_) { /* leave as-is */ }
  }
  renderInputsModal(inputs || {}, protocol || '');
}

// small sanitizer reused by modal

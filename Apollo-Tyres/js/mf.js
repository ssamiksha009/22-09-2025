...
document.getElementById('submitBtn').addEventListener('click', async function() {
    ...
    try {
        ...
        if (pid) {
            const ids = [
              'p1','p2','p3','l1','l2','l3','vel','ia','sa','sr','aspectRatio',
    ...
    // AFTER you compute matrixJson (whatever object you already have)
await saveDraft('MF62', matrixJson /*, optionalInputsOverride */);


    // Check if project already exists before proceeding
    const projectName = sessionStorage.getItem('currentProject') || 'DefaultProject';
    checkProjectExists(projectName, 'MF62');
});
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const pid = getProjectId();
  if (pid) {
        // Try to fetch saved inputs and prefill the form if available
        try {
            const proj = await fetchProject(pid);
            if (proj && proj.project && proj.project.inputs) {
                prefill(proj.project.inputs);
            }
        } catch (e) {
            console.warn('No saved inputs to prefill:', e);
        }
        try { await saveInputs(pid, collectInputs(ids)); } catch (e) { console.error(e); }
      }
    });

    /* ==== DRAFT HELPERS (paste once per file, near top) ==== */

// Small guard so we don't spam the server on every keypress.
function debounce(fn, ms = 600) {
  let t; 
  return (...args) => { clearTimeout(t); t = setTimeout(() => fn(...args), ms); };
}

// Try to resolve projectId consistently with your app.
...
// Collect inputs from this protocol page. Keep selectors aligned to your HTML.
function collectProtocolInputs(containerSelector = 'main') {
  const root = document.querySelector(containerSelector) || document;
  const fields = root.querySelectorAll('input, select, textarea');
  const data = {};
  fields.forEach(el => {
    if (!el.name) return;
    // grab text/number; you can coerce to Number if you prefer
...
/**
 * Call this with your computed matrix object (whatever you already pass to render tables).
 * @param {string} protocol One of "MF62" | "MF52" | "FTire" | "CDTire" | "Custom"
 * @param {object} matrixJson Your computed test matrix (the tables you render on select.html)
 * @param {object} [inputsJson] Page inputs (if omitted, will auto-collect)
 */
async function saveDraft(protocol, matrixJson, inputsJson) {
  try {
    const projectId = await resolveProjectId();
    if (!projectId) return;  // no project yet, skip silently

    const payload = {
      inputs_json: inputsJson || collectProtocolInputs('main'),
      matrix_json: matrixJson || {}
    };

    await fetch(`/api/projects/${encodeURIComponent(projectId)}/protocol/${encodeURIComponent(protocol)}/draft`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    });
    // No toast here on purpose; keep UX quiet.
  } catch (e) {
    // Non-fatal: draft failure should not block user workflow
...
document.addEventListener('DOMContentLoaded', () => {
  const autosave = debounce(async () => {
    const inputs = collectProtocolInputs('main');
    await saveDraft('MF62', /*matrix*/ {}, inputs);
  }, 800);

  // Listen to any change in form inputs on this page

...
/**
 * GET /api/me
 * Return the current user with created_at and last_login.
...
app.post('/api/mark-project-in-progress', async (req, res) => {
  try {
    ...
    `;
    const r = await db.query(q, ['In Progress', projectName]);
    if (r.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Project not found' });
    }
    res.json({ success: true, project: r.rows[0] });
  } catch (err) {
    console.error('mark-project-in-progress error', err);
...
app.post('/api/check-project-exists', async (req, res) => {
  try {
    ...
    // Search by project_name (and protocol if provided)
    let q = 'SELECT id, project_name, protocol FROM projects WHERE project_name = $1';
    const params = [projectName];
    if (protocol) {
      q = 'SELECT id, project_name, protocol FROM projects WHERE project_name = $1 AND protocol = $2';
    ...
    res.json({ success: true, exists, folderName, project: r.rows[0] || null });
  } catch (err) {
...
// Fix PUT /api/projects/:id/inputs to return proper responses
app.put('/api/projects/:id/inputs', async (req, res) => {
  try {
    ...
    const r = await db.query(q, [inputs, id]);
    if (!r.rows || r.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Project not found' });
    }
    return res.json({ success: true, inputs: r.rows[0].inputs });
  } catch (err) {
    console.error('PUT /api/projects/:id/inputs error', err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});
...
app.post('/api/manager/add-user', authenticateToken, requireManager, async (req, res) => {
  try {
    ...
    const insertQ = 'INSERT INTO users (email, password, role, created_at) VALUES ($1,$2,$3,CURRENT_TIMESTAMP) RETURNING id, email, role';
    const r = await db.query(insertQ, [email, hashed, role]);
...
// Manager: list projects for a specific user
app.get('/api/manager/user-projects', authenticateToken, requireManager, async (req, res) => {
  try {
    ...
    console.error('/api/manager/user-projects error', err);
    return res.status(500).json({ success: false, message: 'Server error' });
  }
});

// Manager: list all projects (with optional filters)
app.get('/api/manager/projects', authenticateToken, requireManager, async (req, res) => {
  try {
    const { status, protocol, user_email } = req.query;

    let filters = [];
    let params = [];
    let query = `
      SELECT p.id, p.project_name, p.region, p.department, p.tyre_size, p.protocol, p.status, p.created_at, p.completed_at,
    ...
    // Protocol filter
    if (protocol) {
      filters.push('p.protocol = $' + (params.length + 1));
      params.push(protocol);
    }

    // User email filter (manager view)
    if (user_email) {
      filters.push('p.user_email = $' + (params.length + 1));
    ...
    console.error('/api/manager/projects error', err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});
...
// Archive current scratch rows into *_project_data and mark project completed
app.post('/api/projects/:id/complete', async (req, res) => {
  ...
  try {
    ...
    // 1) Identify protocol & sanity check
    const { rows } = await client.query(
      `SELECT id, protocol FROM projects WHERE id=$1 LIMIT 1`, [id]
    );
    if (!rows.length) throw new Error('Project not found');
    const protocol = rows[0].protocol;

    // 2) Protocol-specific INSERT ... SELECT from scratch → persistent
    if (protocol === 'MF62') {
      await client.query(`
        INSERT INTO mf62_project_data
        (project_id, number_of_runs, tests, ips, loads, inclination_angle, slip_angle, slip_ratio, test_velocity,
         job, old_job, template_tydex, tydex_name, p, l)
        SELECT $1, number_of_runs, tests, ips, loads, inclination_angle, slip_angle, slip_ratio, test_velocity,
               job, old_job, template_tydex, tydex_name, p, l
        FROM mf_data
        WHERE COALESCE(tests,'') <> ''
        ON CONFLICT (project_id, number_of_runs) DO UPDATE SET
          tests=EXCLUDED.tests, ips=EXCLUDED.ips, loads=EXCLUDED.loads,
          inclination_angle=EXCLUDED.inclination_angle, slip_angle=EXCLUDED.slip_angle,
          slip_ratio=EXCLUDED.slip_ratio, test_velocity=EXCLUDED.test_velocity,
          job=EXCLUDED.job, old_job=EXCLUDED.old_job,
          template_tydex=EXCLUDED.template_tydex, tydex_name=EXCLUDED.tydex_name,
          p=EXCLUDED.p, l=EXCLUDED.l, created_at=NOW();
      `, [id]);
    } else if (protocol === 'MF52') {
      await client.query(`
        INSERT INTO mf52_project_data
        (project_id, number_of_runs, tests, inflation_pressure, loads, inclination_angle, slip_angle, slip_ratio, test_velocity,
         job, old_job, template_tydex, tydex_name, p, l)
        SELECT $1, number_of_runs, tests, inflation_pressure, loads, inclination_angle, slip_angle, slip_ratio, test_velocity,
               job, old_job, template_tydex, tydex_name, p, l
        FROM mf52_data
        WHERE COALESCE(tests,'') <> ''
        ON CONFLICT (project_id, number_of_runs) DO UPDATE SET
          tests=EXCLUDED.tests, inflation_pressure=EXCLUDED.inflation_pressure, loads=EXCLUDED.loads,
          inclination_angle=EXCLUDED.inclination_angle, slip_angle=EXCLUDED.slip_angle,
          slip_ratio=EXCLUDED.slip_ratio, test_velocity=EXCLUDED.test_velocity,
          job=EXCLUDED.job, old_job=EXCLUDED.old_job,
          template_tydex=EXCLUDED.template_tydex, tydex_name=EXCLUDED.tydex_name,
      ...
      await client.query(`
        ...
        (project_id, number_of_runs, tests, loads, inflation_pressure, test_velocity, longitudinal_slip, slip_angle,
         inclination_angle, cleat_orientation, job, old_job, template_tydex, tydex_name, p, l)
        SELECT $1, number_of_runs, tests, loads, inflation_pressure, test_velocity, longitudinal_slip, slip_angle,
               inclination_angle, cleat_orientation, job, old_job, template_tydex, tydex_name, p, l
        FROM ftire_data
        WHERE COALESCE(tests,'') <> ''
        ON CONFLICT (project_id, number_of_runs) DO UPDATE SET
          tests=EXCLUDED.tests, loads=EXCLUDED.loads, inflation_pressure=EXCLUDED.inflation_pressure,
          test_velocity=EXCLUDED.test_velocity, longitudinal_slip=EXCLUDED.longitudinal_slip,
          slip_angle=EXCLUDED.slip_angle, inclination_angle=EXCLUDED.inclination_angle,
          cleat_orientation=EXCLUDED.cleat_orientation, job=EXCLUDED.job, old_job=EXCLUDED.old_job,
          template_tydex=EXCLUDED.template_tydex, tydex_name=EXCLUDED.tydex_name,
          p=EXCLUDED.p, l=EXCLUDED.l, created_at=NOW();
      `, [id]);
      ...
      await client.query(`
        ...
        (project_id, number_of_runs, test_name, inflation_pressure, velocity, preload, camber, slip_angle, displacement,
         slip_range, cleat, road_surface, job, old_job, template_tydex, tydex_name, p, l)
        SELECT $1, number_of_runs, test_name, inflation_pressure, velocity, preload, camber, slip_angle, displacement,
               slip_range, cleat, road_surface, job, old_job, template_tydex, tydex_name, p, l
        FROM cdtire_data
        WHERE COALESCE(test_name,'') <> ''
        ON CONFLICT (project_id, number_of_runs) DO UPDATE SET
          test_name=EXCLUDED.test_name, inflation_pressure=EXCLUDED.inflation_pressure, velocity=EXCLUDED.velocity,
          preload=EXCLUDED.preload, camber=EXCLUDED.camber, slip_angle=EXCLUDED.slip_angle, displacement=EXCLUDED.displacement,
          slip_range=EXCLUDED.slip_range, cleat=EXCLUDED.cleat, road_surface=EXCLUDED.road_surface,
          job=EXCLUDED.job, old_job=EXCLUDED.old_job, template_tydex=EXCLUDED.template_tydex, tydex_name=EXCLUDED.tydex_name,
          p=EXCLUDED.p, l=EXCLUDED.l, created_at=NOW();
      `, [id]);
    } else { // Custom
      await client.query(`
        INSERT INTO custom_project_data
        (project_id, number_of_runs, tests, inflation_pressure, loads, inclination_angle, slip_angle, slip_ratio,
         test_velocity, cleat_orientation, displacement, job, old_job, template_tydex, tydex_name, p, l)
        SELECT $1, number_of_runs, tests, inflation_pressure, loads, inclination_angle, slip_angle, slip_ratio,
               test_velocity, cleat_orientation, displacement, job, old_job, template_tydex, tydex_name, p, l
        FROM custom_data
        WHERE COALESCE(tests,'') <> ''
        ON CONFLICT (project_id, number_of_runs) DO UPDATE SET
          tests=EXCLUDED.tests, inflation_pressure=EXCLUDED.inflation_pressure, loads=EXCLUDED.loads,
          inclination_angle=EXCLUDED.inclination_angle, slip_angle=EXCLUDED.slip_angle, slip_ratio=EXCLUDED.slip_ratio,
          test_velocity=EXCLUDED.test_velocity, cleat_orientation=EXCLUDED.cleat_orientation, displacement=EXCLUDED.displacement,
          job=EXCLUDED.job, old_job=EXCLUDED.old_job, template_tydex=EXCLUDED.template_tydex, tydex_name=EXCLUDED.tydex_name,
          p=EXCLUDED.p, l=EXCLUDED.l, created_at=NOW();
      `, [id]);
    }
    ...
    await client.query(`UPDATE projects SET status='Completed', completed_at=NOW() WHERE id=$1`, [id]);
...
// === Archived data reader (generic by project id) ===
app.get('/api/projects/:id/data', async (req, res) => {
  try {
    const { id } = req.params;

    // Look up the protocol for this project
    const pr = await db.query('SELECT protocol FROM projects WHERE id = $1', [id]);
    if (!pr.rows.length) {
      return res.status(404).json({ success: false, message: 'Project not found' });
    }
    const protocol = pr.rows[0].protocol;

    // Choose the right archive table
    let sql;
    switch (protocol) {
      case 'MF62':
        sql = 'SELECT * FROM mf62_project_data WHERE project_id = $1 ORDER BY number_of_runs';
        break;
      case 'MF52':
        sql = 'SELECT * FROM mf52_project_data WHERE project_id = $1 ORDER BY number_of_runs';
        break;
      case 'FTire':
        sql = 'SELECT * FROM ftire_project_data WHERE project_id = $1 ORDER BY number_of_runs';
        break;
      case 'CDTire':
        sql = 'SELECT * FROM cdtire_project_data WHERE project_id = $1 ORDER BY number_of_runs';
        break;
      case 'Custom':
        sql = 'SELECT * FROM custom_project_data WHERE project_id = $1 ORDER BY number_of_runs';
        break;
      default:
        return res.status(400).json({ success: false, message: `Unknown protocol: ${protocol}` });
    }

    const r = await db.query(sql, [id]);
    return res.json({ success: true, protocol, rows: r.rows || [] });
  } catch (err) {
...
// ================================================
// In-Progress Drafts + Tydex text endpoints
// ================================================

// Upsert draft (called whenever you compute matrix/tydex while status="In Progress")
app.post('/api/projects/:projectId/protocol/:protocol/draft', async (req, res) => {
  const { projectId, protocol } = req.params;
  const { inputs_json, matrix_json } = req.body; // send both from FE
  try {
    const q = `
      ...
      DO UPDATE SET inputs_json = EXCLUDED.inputs_json,
                    ...
                    updated_at = NOW()
      RETURNING id, project_id, protocol, inputs_json, matrix_json, updated_at;
    `;
    const { rows } = await db.query(q, [projectId, protocol, inputs_json, matrix_json]);
...
// Fetch draft to resume
app.get('/api/projects/:projectId/protocol/:protocol/draft', async (req, res) => {
  const { projectId, protocol } = req.params;
  try {
    const { rows } = await db.query(
      `SELECT id, project_id, protocol, inputs_json, matrix_json, updated_at
         FROM protocol_drafts
        WHERE project_id=$1 AND protocol=$2`,
      [projectId, protocol]
    );
    if (!rows.length) return res.json({ ok: true, draft: null });
    res.json({ ok: true, draft: rows[0] });
  } catch (e) {
...
// Save a newly generated Tydex text (call each time you "Generate" a Tydex)
app.post('/api/projects/:projectId/protocol/:protocol/tydex', async (req, res) => {
  const { projectId, protocol } = req.params;
  const { file_name, file_text } = req.body;
  if (!file_name || !file_text) {
    return res.status(400).json({ ok: false, error: 'file_name and file_text required' });
  }

  try {
    const { rows } = await db.query(
      `INSERT INTO tydex_files (project_id, protocol, file_name, file_text)
       VALUES ($1, $2, $3, $4)
       RETURNING id, project_id, protocol, file_name, created_at`,
      [projectId, protocol, file_name, file_text]
    );
    res.json({ ok: true, file: rows[0] });
  } catch (e) {
    console.error('SAVE TYDEX ERROR:', e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// List previously generated Tydex files for this project+protocol
app.get('/api/projects/:projectId/protocol/:protocol/tydex', async (req, res) => {
  const { projectId, protocol } = req.params;
  try {
    ...
    console.error('LIST TYDEX ERROR:', e);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Download/view Tydex text
app.get('/api/projects/:projectId/protocol/:protocol/tydex/:fileId', async (req, res) => {
  const { projectId, protocol, fileId } = req.params;
  try {
    const { rows } = await pool.query(
      `SELECT file_name, file_text
         FROM tydex_files
        WHERE id=$1 AND project_id=$2 AND protocol=$3`,
      [fileId, projectId, protocol]
    );
    if (!rows.length) return res.status(404).send('Not found');
    res.set('Content-Type', 'text/plain; charset=utf-8');
    res.set('Content-Disposition', `attachment; filename="${rows[0].file_name}"`);
    res.send(rows[0].file_text);
  } catch (e) {
    console.error('DOWNLOAD TYDEX ERROR:', e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
